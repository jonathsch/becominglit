...
from becominglit.util.lbs import blend_shapes, lbs, vertices2landmarks
...
class FlameParamHolder(nn.Module):
    def __init__(self, flame_params: Dict[str, torch.Tensor]):
        super().__init__()
        self.flame_params = nn.ParameterDict(
            {
                k: nn.Parameter(v.clone())
                for k, v in flame_params.items()
                if k
                in ["shape", "expr", "rotation", "neck_pose", "jaw_pose", "eyes_pose", "translation", "static_offset"]
            }
        )

    def forward(self, indices: torch.Tensor, use_static_offset: bool = True):
        flame_batch = {
            k: v[indices] for k, v in self.flame_params.items() if k not in {"shape", "static_offset", "hair_offset"}
        }
        flame_batch["shape"] = self.flame_params["shape"].repeat(len(indices), 1)
        if use_static_offset:
            flame_batch["static_offset"] = self.flame_params["static_offset"].clone()
        else:
            flame_batch["static_offset"] = torch.zeros_like(self.flame_params["static_offset"])
        return flame_batch
...
class FlameHead(nn.Module):
    ...
    def __init__(
        ...
        self.register_buffer("face_uvcoords", face_uv_coords, persistent=True)
        self.register_buffer("faces", faces.verts_idx, persistent=True)

        self.register_buffer("verts_uvs", aux.verts_uvs, persistent=True)
        self.register_buffer("textures_idx", faces.textures_idx, persistent=True)
...
class FlameTexPainted(nn.Module):
    def __init__(self, tex_size=512, painted_tex_path=FLAME_PAINTED_TEX_PATH):
        ...
        if tex_painted.shape[-1] != self.tex_size or tex_painted.shape[-2] != self.tex_size:
            tex_painted = thf.interpolate(tex_painted, [self.tex_size, self.tex_size])
...
class FlameTexPCA(nn.Module):
    ...
    def forward(self, texcode):
        ...
        texture = thf.interpolate(texture, [self.tex_size, self.tex_size])
...
class FlameMask(nn.Module):
    ...
    def create_custom_mask(self):
        ...
        self.v.register_buffer(
            ...
            torch.tensor([1595, 1746, 1747, 1742, 1739, 1665, 1666, 3514, 2783, 2782, 2854, 2857, 2862, 2861, 2731]),
        )

        self.v.register_buffer(
            "lip_inside_ring_lower",
            torch.tensor([1572, 1573, 1860, 1862, 1830, 1835, 1852, 3497, 2941, 2933, 2930, 2945, 2943, 2709, 2708]),
        )

        self.v.register_buffer(
            "lip_outside_ring_upper",
            torch.tensor([1713, 1715, 1716, 1735, 1696, 1694, 1657, 3543, 2774, 2811, 2813, 2850, 2833, 2832, 2830]),
        )

        self.v.register_buffer(
            "lip_outside_ring_lower",
            torch.tensor([1576, 1577, 1773, 1774, 1795, 1802, 1865, 3503, 2948, 2905, 2898, 2881, 2880, 2713, 2712]),
        )
        ...
        self.v.register_buffer(
            "lip_inside_ring",
            torch.cat([self.v.lip_inside_ring_upper, self.v.lip_inside_ring_lower, torch.tensor([1594, 2730])]),
        )
