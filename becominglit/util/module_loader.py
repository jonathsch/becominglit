        ...
        if "per_module" in config:
            ...
            for name, value in config.per_module.items():
                if not hasattr(model, name):
                    raise RuntimeError(f"model {model.__class__} does not have a submodule {name}")

                if isinstance(getattr(model, name), nn.Parameter):
                    params.append(
                        dict(
            ...
            for name, module in model.named_children():
                ...
                if name not in defined_names and n_params:
                    logger.warning(f"not going to optimize module {name} which has {n_params} parameters")
