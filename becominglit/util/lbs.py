...
def lbs(
    ...
    J[:, 0, :] = 0.0  # NOTE: We set the root joint to the origin of the canonical mesh coordinate system
...
def transform_mat(R, t):
    ...
    return torch.cat([thf.pad(R, [0, 0, 0, 1]), thf.pad(t, [0, 0, 0, 1], value=1)], dim=2)
...
def batch_rigid_transform(rot_mats, joints, parents, dtype=torch.float32):
    ...
    joints_homogen = thf.pad(joints, [0, 0, 0, 1])

    rel_transforms = transforms - thf.pad(torch.matmul(transforms, joints_homogen), [3, 0, 0, 0, 0, 0, 0, 0])
