...
from becominglit.util import vecmath as V
...
class NVDiffRenderer(torch.nn.Module):
    ...
    def projection_from_intrinsics(
        self, K: torch.Tensor, image_size: Tuple[int], near: float = 0.001, far: float = 100
    ):
        """
    ...
    def compute_v_normals(self, verts, faces):
        ...
        v_normals = torch.where(
            V.dot(v_normals, v_normals) > 1e-20,
            v_normals,
    ...
    def shade(self, normal, lighting_coeff=None):
        if self.lighting_type == "constant":
            ...
            bias = 0.75
            diffuse = torch.clamp(
                V.dot(normal, torch.tensor([0.0, 0.0, 1.0], dtype=torch.float32, device="cuda")) + bias, 0.0, 1.0
            )
    ...
    def render_rgba(
        ...
        if enable_disturbance:
            ...
            for i in range(num_clusters):
                ...
                c_mask = cid == i
                c_pixels = (
                    c_rgba[c_mask.repeat_interleave(4, dim=-1)].reshape(-1, 4).detach()
                )  # NOTE: detach to avoid gradient flow
