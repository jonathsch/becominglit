...
def weight_norm_wrapper(
    ...
    class Wrap(cls):
        def __init__(self, *args: Any, name=name, g_dim=g_dim, v_dim=v_dim, **kwargs: Any):
            ...
            check_args_shadowing("weight_norm_wrapper", super().__init__, ["name", "g_dim", "v_dim"])
        ...
        def unfuse(self):
            ...
            wn = WeightNorm.apply(self, self.weight_norm_args["name"], self.weight_norm_args["g_dim"])
...
class Conv2dUB(th.nn.Conv2d):
    ...
    def _conv_forward(self, input: th.Tensor, weight: th.Tensor, bias: Optional[th.Tensor]):
        ...
        if self.padding_mode != "zeros":
            ...
            return thf.conv2d(input, weight, bias, self.stride, _pair(0), self.dilation, self.groups)
        return thf.conv2d(
            input,
            weight,
            bias,
...
# pyre-fixme[5]: Global expression must be annotated.
ConvTranspose2dWN = weight_norm_wrapper(th.nn.ConvTranspose2d, "ConvTranspose2dWN", g_dim=1, v_dim=None)
# pyre-fixme[5]: Global expression must be annotated.
ConvTranspose2dWNUB = weight_norm_wrapper(ConvTranspose2dUB, "ConvTranspose2dWNUB", g_dim=1, v_dim=None)
    ...
    class Wrap(cls):
        def __init__(
            ...
            **kwargs: Any,
            ...
            super().__init__(*args, **kwargs)
            self.register_forward_pre_hook(InterpolateHook(size=size, scale_factor=scale_factor, mode=mode))
...
class Conv2dELR(th.nn.Module):
    def __init__(
        ...
        if bias:
            if untied:
                self.bias: th.nn.Parameter = th.nn.Parameter(th.zeros(out_channels, height, width, dtype=th.float32))
            else:
                self.bias: th.nn.Parameter = th.nn.Parameter(th.zeros(out_channels, dtype=th.float32))
        else:
            self.register_parameter("bias", None)
        self.untied = untied
        self.std: float = 0.0
...
class ConcatPyramid(th.nn.Module):
    ...
    def forward(self, x, y):
        if self.transposed:
            blurred = thf.conv2d(y, self.blur_kernel, groups=self.n_concat_in, padding=self.ksize // 2)
            pyramid = [blurred[:, :, ::2, ::2]]
        else:
            pyramid = [y]
...
class Downsample(th.nn.Module):
    ...
    def forward(self, inp):
        if self.filt_size == 1:
            ...
            return th.nn.functional.conv2d(self.pad(inp), self.filt, stride=self.stride, groups=inp.shape[1])
