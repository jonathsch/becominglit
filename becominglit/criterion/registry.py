...
def register_loss(loss_name: str, *outputs):
    ...
    global loss_registry  # noqa: PLW0602
    ...
    def _register(fn):
        if loss_name in loss_registry:
            ...
            logger.warning(
                "Tried to register function for %s which has already been registered as %s" % (loss_name, existing)
            )
...
class FnLoss(nn.Module):
    ...
    def forward(self, preds: Dict[str, Any], targets: Dict[str, Any]) -> Union[torch.Tensor, float]:
...
def register_loss_by_fn(loss_name=None, *outputs):
    ...
    def _register(fn):
        ...
        # pyre-ignore
        loss_registry[loss_name] = lambda **function_args: FnLoss(fn, function_args)
        return fn
