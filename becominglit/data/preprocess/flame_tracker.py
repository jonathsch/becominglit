...
from becominglit.data.preprocess.tracking_config import PhotometricStageConfig, TrackingConfig
from becominglit.data.preprocess.tracking_dataset import BecomingLitTrackingDataset
from becominglit.data.preprocess.utils import normalize_image_points, plot_landmarks_2d
from becominglit.util.flame import FlameHead, FlameTexPainted, FlameTexPCA, FlameUvMask
...
class FlameTracker:
    ...
    def load_shape_params(self, fp):
        """
        Initializes the shape parameters from another tracking result.
        """
        shape_code = np.load(fp)["shape"]
        self.shape.data = torch.from_numpy(shape_code).to(self.shape.device)
    ...
    def load_from_tracked_flame_params(self, fp):
        ...
        if not self.cfg.model.tex_painted:
            if "tex" in report:
                ...
                logger.warning("No tex_extra found in flame_params!")
    ...
    def get_current_frame(self, frame_idx, include_keyframes=False):
        ...
        if hasattr(self.dataset, "batch_filter_fn"):
            sample = to_device(sample, self.device)
            self.dataset.batch_filter_fn(sample)
    ...
    def configure_optimizer(self, params, lr_scale=1.0):
        ...
        if self.cfg.model.shape_init_path is not None:
            group_lr["shape"] = 0.0
    ...
    def rasterize_flame(self, sample, verts, faces, camera_index=None, train_mode=False):
        """
    ...
    def render_rgba(
        ...
        if self.cfg.render.backend == "nvdiffrast":
            ...
            rgba = self.render.render_rgba(rast_dict, verts, faces, verts_uv, faces_uv, tex, lights, background_color)
    ...
    def compute_lmk_energy(self, sample, pred_lmks, disable_jawline_landmarks=False):
        ...
        lmk2d = lmk2d[:, :, :2]  # , lmk2d[:, :, 2]
        confidence = torch.ones_like(lmk2d[:, :, 1])
        lmk2d[:, :, 0], lmk2d[:, :, 1] = normalize_image_points(lmk2d[:, :, 0], lmk2d[:, :, 1], img_size)
    ...
    def compute_photometric_energy(
        ...
        align_texture_except_fid = (
            self.flame.mask.get_fid_by_region(self.cfg.pipeline[stage].align_texture_except)
            if stage is not None
            else None
        )
        align_boundary_except_vid = (
            self.flame.mask.get_vid_by_region(self.cfg.pipeline[stage].align_boundary_except)
            if stage is not None
            else None
        )
        ...
        render_out = self.render_rgba(
            ...
            align_texture_except_fid,
            align_boundary_except_vid,
            enable_disturbance=stage is not None,
        )
    ...
    def compute_regularization_energy(
        self, result_dict, verts, verts_cano, lmks, albedos, frame_idx, include_keyframes, stage
    ):
        """
        ...
        if self.opt_dict["static_offset"] or self.opt_dict["dynamic_offset"]:
            if self.static_offset is not None or self.dynamic_offset is not None:
                ...
                if self.cfg.w.reg_offset_lap is not None:
                    ...
                    reg_offset_lap = self.compute_laplacian_smoothing_loss(vert_wo_offset, vert_wo_offset + offset)
    ...
    def compute_energy(
        ...
        if stage is None or isinstance(self.cfg.pipeline[stage], PhotometricStageConfig):
            if self.cfg.w.photo is not None:
                ...
                rast_dict = self.rasterize_flame(sample, verts_n, self.flame.faces, train_mode=True)
    ...
    def log_media(
        ...
        img = self.visualize_tracking(
            verts, lmks, albedos, output_dict, sample, disable_jawline_landmarks=disable_jawline_landmarks
        )
    ...
    def visualize_tracking(
        ...
        # view_indices = torch.arange(len(output_dict["gt_rgb"])) if "gt_rgb" in output_dict else view_indices
        ...
        if "pred_alpha" in output_dict:
            ...
            if diffuse is not None:
                fg_mask *= diffuse
                w = 0.7
            overlay_alpha = fg_mask * (w * color_alpha * pred_alpha + (1 - w) * gt_rgb) + (1 - fg_mask) * gt_rgb
        ...
        if "error_alpha" in output_dict:
            ...
            error_alpha = torch.from_numpy(error_alpha[..., :3]).to(gt_rgb).permute(0, 3, 1, 2)
    ...
    def save_result(self, fname=None, epoch=None):
        ...
        # export albedo texture (png)
        albedo = self.get_albedo().detach().cpu()
        # ensure shape (3, H, W)
        if albedo.ndim == 4 and albedo.shape[0] == 1:
            albedo = albedo[0]
        albedo = torch.clamp(albedo, 0.0, 1.0)

        # determine filename (match npz naming below)
        png_base = fname if fname is not None else "tracked_flame_params"
        if epoch is not None:
            png_base = f"{png_base}_{epoch}"
        png_path = self.out_dir / f"{png_base}_albedo.png"

        save_image(albedo, str(png_path))
...
class GlobalTracker(FlameTracker):
    def __init__(self, cfg: TrackingConfig):
        ...
        # data
        self.dataset = BecomingLitTrackingDataset(
            **asdict(cfg.data),
            img_to_tensor=True,
        ...
        self.image_size = (
            self.dataset[0]["rgb"].shape[-3] // self.cfg.data.n_downsample_rgb,
            self.dataset[0]["rgb"].shape[-2] // self.cfg.data.n_downsample_rgb,
        )
        ...
        if self.cfg.model.shape_init_path is not None:
            self.load_shape_params(self.cfg.model.shape_init_path)
    ...
    def optimize(self):
        ...
        for sample in dataloader:
            sample = to_device(sample, self.device)
            if hasattr(dataloader.dataset, "batch_filter_fn"):
                dataloader.dataset.batch_filter_fn(sample)
        ...
        logger.info("Start global optimization of all frames")
    ...
    def optimize_stage(
        ...
        if sample is not None:
            ...
            for epoch_i in range(num_epochs):
                ...
                for step_i, sample in enumerate(dataloader):  # noqa: PLR1704 TODO: Can this be simplified?
                    sample = to_device(sample, self.device)
                    if hasattr(dataloader.dataset, "batch_filter_fn"):
                        dataloader.dataset.batch_filter_fn(sample)
    ...
    def optimize_iter(self, sample, optimizer, stage):
        ...
        if (self.global_step == 0) or (self.global_step + 1) % self.log_interval_media == 0:
            self.log_media(
                verts,
                faces,
                lmks,
